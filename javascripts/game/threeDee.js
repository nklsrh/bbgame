...
env[1] = collada.scene;
env_skin[1] = collada.skins[ 0 ];
env[1].dynamic = true;
env[1].scale.x = env[1].scale.y = env[1].scale.z = 1;
env[1].position.x = env[0].position.x + 350 ;
env[1].updateMatrix();
...
camera.position.x = player.position.x + 20;//= Math.cos( timer ) * 50;
camera.position.y = 3;
...
if(player.position.x > env[0].position.x && env[1].position.x < env[0].position.x){
  env[1].position.x = player.position.x + 347;
}
else {
  if(player.position.x > env[1].position.x && env[0].position.x < env[1].position.x){
    env[0].position.x = player.position.x + 347;
  }
}
